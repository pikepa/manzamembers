...
<div class="flex justify-between md:items-center flex-col md:flex-row">
    <div class="field w-1/2  mb-6">
        ...
        <div class="w-1/2  block mb-6 ml-4">
              <span class=" font-bold text-gray-800">Event Status</span>
              <div class="mt-1">
                <div class=" flex flex-wrap ">
                  <div class="inline-block relative w-full">
                    <select name="status" class="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                      ...
                      <option value="{{ $status }}" @if (old('status') ==  $status |  $event->status  ==  $status ) {{ 'selected' }} @endif>{{ $status }}</option>
                    @endforeach
                    </select>
                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                      <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
